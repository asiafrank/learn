...
{
    //static TCHAR szFilter[] = L"Text Files (*.TXT)\0*.txt\0ASCII Files (*.ASC)\0*.asc\0All Files (*.*)\0*.*\*\0\0";
...
BOOL PopFileSaveDlg(HWND hwnd, PTSTR pstrFileName, PTSTR pstrTiteName)
{
    ofn.hwndOwner      = hwnd;
    ofn.lpstrFile      = pstrFileName;
    ofn.lpstrFileTitle = pstrTiteName;
...
BOOL PopFileRead(HWND hwndEdit, PTSTR pstrFileName)
{
    ...
    if (IsTextUnicode(pBuffer, iFileLength, &iUniTest))
    {
        ...
        // Alocate memory for possibly converted string
...
#else  // If the edit control is Unicode, just copy the string
...
{
    DWORD dwBytesWriteen;
    HANDLE hFile;
    int iLength;
    PTSTR pstrBuffer;
    WORD wByteOrderMark = 0xFEFF;

    // Open the file creatint it if necessary
...
#ifdef UNICODE
    WriteFile(hFile, &wByteOrderMark, 2, &dwBytesWriteen, NULL);
#endif // UNICODE
    // Get the edit buffer and write that out to the file.
    GetWindowText(hwndEdit, pstrBuffer, iLength + 1);
    WriteFile(hFile, pstrBuffer, iLength * sizeof(TCHAR), &dwBytesWriteen, NULL);

    if ((iLength * sizeof(TCHAR)) != (int)dwBytesWriteen)
    {
        CloseHandle(hFile);
        free(pstrBuffer);
        return FALSE;
