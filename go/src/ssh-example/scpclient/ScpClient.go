...
func (a *ScpClient) CopyFromFile(file os.File, remotePath string, permissions string) error {
	return a.CopyFromFilePassThru(file, remotePath, permissions, nil)
}

// Copies the contents of an os.File to a remote location, it will get the length of the file by looking it up from the filesystem.
// Access copied bytes by providing a PassThru reader factory
func (a *ScpClient) CopyFromFilePassThru(file os.File, remotePath string, permissions string, passThru PassThru) error {
	stat, _ := file.Stat()
...
// if the file length in know in advance please use "Copy" instead.
// Access copied bytes by providing a PassThru reader factory
func (a *ScpClient) CopyFilePassThru(fileReader io.Reader, remotePath string, permissions string, passThru PassThru) error {
...
func (a *ScpClient) Copy(r io.Reader, remotePath string, permissions string, size int64) error {
	return a.CopyPassThru(r, remotePath, permissions, size, nil)
}

// Copies the contents of an io.Reader to a remote location.
// Access copied bytes by providing a PassThru reader factory
func (a *ScpClient) CopyPassThru(r io.Reader, remotePath string, permissions string, size int64, passThru PassThru) error {
	if passThru != nil {
		r = passThru(r, size)
	}
	...
	go func() {
		defer wg.Done()
		// FIXME: 跳板机连接后，执行 scp -qt remotePath 出现 EOF 错误
