# 例子程序

[go语言之旅](https://tour.go-zh.org/)

# 程序列表

- [hello.go](hello.go)

- [if.go](if.go)
- [if-and-else.go](if-and-else.go)
- [if-with-a-short-statement.go](if-with-a-short-statement.go)

- [exercise-loops-and-functions.go](exercise-loops-and-functions.go)

- [defer.go](defer.go)
- [defer-multi.go](defer-multi.go)

- [pointers.go](pointers.go)
- [structs.go](structs.go)
- [struct-fields.go](struct-fields.go) 
- [struct-pointers.go](struct-pointers.go)
- [struct-literals.go](struct-literals.go)
- [array.go](array.go)
- [slices.go](slices.go)
- [slices-pointers.go](slices-pointers.go)
- [slice-literals.go](slice-literals.go)
- [slice-bounds.go](slice-bounds.go)
- [slice-len-cap.go](slice-len-cap.go)
- [nil-slices](nil-slices.go)
- [making-slices.go](making-slices.go)
- [slices-of-slice.go](slices-of-slice.go)
- [append.go](append.go)
- [range.go](range.go)
- [range-continued.go](range-continued.go)
- [exercise-slices.go](exercise-slices.go)
- [maps.go](maps.go)
- [map-literals.go](map-literals.go)
- [map-literals-continued.go](map-literals-continued.go)